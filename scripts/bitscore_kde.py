...
df = pd.read_csv(f'{basedir}/results/risultati_finali.tsv', sep = '\t')
df.columns = ['N', 'Sequence', 'RPM', 'SeqID', 'Best P-match', 'Plant miRs',
       'Best HSa-match', 'Hsa miRs', 'Best O-match', 'Other miRs',
       'Best-HRefSeq', 'Hsa RefSeq', 'RefSeq-Ident', 'RefSeq-Gaps']
...
plt.ylabel('')
plt.savefig(f'{basedir}/graphs/bitscores_distribution.png', dpi = 600)
plt.clf()

sns.kdeplot(data = dataf, x = 'Bitscore', hue = 'Database', fill = True, cumulative = True)
plt.ylabel('')
plt.savefig(f'{basedir}/graphs/bitscores_distribution_cumulative.png', dpi = 600)
plt.clf()
